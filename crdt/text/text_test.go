...
func TestExecuteOp(t *testing.T) {
	...
	assert.Equal(t, Edit{0, 0, "Hěllo Ťhere"}, e1[0])
	assert.Equal(t, Edit{0, 13, "Hěllo ere"}, e2[0])
	assert.Equal(t, Edit{0, 10, "Hěllo erstwhile"}, e3[0])
	e1[0].tryMerge(e2[0].Index, e2[0].Count, e2[0].Text)
	e1[0].tryMerge(e3[0].Index, e3[0].Count, e3[0].Text)
	assert.Equal(t, Edit{0, 0, "Hěllo erstwhile"}, e1[0])
}

func TestExecuteOpDupe(t *testing.T) {
	x1 := New(1)
	x2 := x1.Replicate(2)
